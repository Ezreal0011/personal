...
const Canvas = {
    ...
    _createNodeElement(node, project) {
        ...
        if (this.selectedNode && project) {
            ...
            const isSelectedRoot = selectedNode && selectedNode.type === 'system' && !selectedNode.parentId;
            
            if (isSelectedRoot) {
                // 选中根级父节点：所有子节点高亮（通过parentId查找）
                const isChild = node.parentId === this.selectedNode;
                if (isChild) {
                    isHighlighted = true;
                } else {
                    isDimmed = true;
                ...
                // 一级关联节点高亮，其他变暗
                const related = project.edges.some(e => 
                    (e.source === this.selectedNode && e.target === node.id) ||
                    (e.target === this.selectedNode && e.source === node.id)
                );
                if (related) {
                    isHighlighted = true;
                } else {
    ...
    selectNode(nodeId) {
        ...
        const isRootParent = selectedNode && selectedNode.type === 'system' && !selectedNode.parentId;

        this.overlay.querySelectorAll('.canvas-node').forEach(c => {
            const id = c.dataset.nodeId;
            
            if (isRootParent) {
                // 根级父节点：高亮所有子节点（通过parentId查找）
                const childNode = project.nodes.find(n => n.id === id);
                const isChild = childNode && childNode.parentId === nodeId;
                c.classList.toggle('path-highlight-node', isChild);
                c.classList.toggle('path-dimmed-node', !isChild);
            } else {
                // 普通节点
