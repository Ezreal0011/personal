...
const Canvas = {
    ...
    _createResourceNode(node, project, isSelected) {
        ...
        // 自动计算输入的资源数量（从连接到该资源节点的边）
        const inputEdges = project.edges.filter(e => e.target === node.id && (e.type === 'output' || e.type === 'consume'));
        let totalInput = 0;
        inputEdges.forEach(edge => {
            if (edge.valueType === 'fixed' && edge.value != null) totalInput += parseInt(edge.value);
            else if (edge.valueType === 'range' && edge.maxValue != null) totalInput += parseInt(edge.maxValue);
        });
        const qtyHtml = totalInput > 0 ? `<span style="font-weight:bold;font-size:14px;color:var(--text-primary);">x${totalInput}</span>` : '';
