...
const Canvas = {
    ...
    _renderEdgesWithRealPositions(project) {
        ...
        for (const edge of project.edges) {
            ...
            html += `<path class="${styleClass}" d="${path}" stroke="${color}" stroke-width="${strokeWidth}" stroke-opacity="${opacity}" fill="none" marker-end="url(#arrow-${edge.type || 'default'})" data-edge-id="${edge.id}" style="cursor:pointer;${isDimmed ? 'opacity:0.15;' : ''}" onclick="Canvas.selectEdge('${edge.id}')" oncontextmenu="Canvas._onEdgeContextMenu(event, '${edge.id}')"/>`;
